...
<div class="row mt-3">
  <div class="col-md-12">
        <div class="card">
            <form class="form" action="{{ route('postStore') }}" method="post">
                @csrf
                <div class="card-body row g-3">
                    ...
                    <div class="">
                        ...
                        <textarea name="content" id="content" rows="3" class="form-control  @error ('content') is-invalid @enderror" placeholder="{{ __('المحتوى') }}">{{ old('content') }}</textarea>
                        @error('content')
                            <div class="invalid-feedback form-text fw-semibold fs-6">{{ $message }}</div>
                        @enderror
                    </div>
