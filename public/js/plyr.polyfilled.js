typeof navigator === "object" &&
  (function (global, factory) {
    ...
    function closest$1(element, selector) {
      const { prototype } = Element; // https://developer.mozilla.org/en-US/docs/Web/API/Element/closest#Polyfill
    ...
    const standardRatios = [
      ...
      [32, 9],
      [9, 32],
    ].reduce((out, [x, y]) => ({ ...out, [x / y]: [x, y] }), {}); // Validate an aspect ratio
    ...
    function getAspectRatio(input) {
      const parse = (ratio) =>
        validateAspectRatio(ratio) ? ratio.split(":").map(Number) : null; // Try provided ratio
      ...
      updateProgress(event) {
        ...
        const setProgress = (target, input) => {
          ...
          const progress = is.element(target)
            ? target
            : this.elements.display.buffer; // Update value and label
      ...
      create(data) {
        ...
        dedupe(
          ...
          if (control === "mute" || control === "volume") {
            let { volume } = this.elements; // Create the volume container if needed
    ...
    const captions = {
      ...
      set(index, passive = true) {
        ...
        if (this.captions.currentTrack !== index) {
          ...
          const { language } = track || {}; // Store reference to node for invalidation on remove
        ...
        _defineProperty$1(this, "onChange", () => {
          ...
          const target =
            this.target === this.player.media
              ? this.target
              : this.player.elements.container; // Trigger an event
    ...
    class Listeners {
      constructor(_player) {
        ...
        _defineProperty$1(this, "global", (toggle = true) => {
          const { player } = this; // Keyboard shortcuts
        ...
        _defineProperty$1(this, "container", () => {
          const { player } = this;
          const { config, elements, timers } = player; // Keyboard shortcuts
          ...
          on.call(
            ...
            (event) => {
              const { controls: controlsElement } = elements; // Remove button states for fullscreen
          ...
          on.call(
            ...
            (event) => {
              const { target } = player.fullscreen; // Ignore events not from target
        ...
        _defineProperty$1(this, "media", () => {
          const { player } = this;
          const { elements } = player; // Time change on media
          ...
          on.call(player, player.media, proxyEvents, (event) => {
            let { detail = {} } = event; // Get error details from media
        ...
        _defineProperty$1(this, "controls", () => {
          const { player } = this;
          const { elements } = player; // IE doesn't support input event, so we fallback to change
          ...
          this.bind(elements.controls, "focusin", () => {
            const { config, timers } = player; // Skip transition to prevent focus from scrolling the parent element
    ...
    const vimeo = {
      ...
      ready() {
        ...
        const { premium, referrerPolicy, ...frameParams } = config; // Get the source URL or ID
    ...
    const youtube = {
      ...
      getTitle(videoId) {
        ...
        fetch(url)
          .then((data) => {
            if (is.object(data)) {
              const { title, height, width } = data; // Set title
      ...
      ready() {
        ...
        if (config.customControls) {
          const posterSrc = (s) =>
            `https://i.ytimg.com/vi/${videoId}/${s}default.jpg`; // Check thumbnail images in order of quality, but reject fallback thumbnails (120px wide)
    ...
    class Ads {
      ...
      constructor(player) {
        ...
        _defineProperty$1(this, "onAdEvent", (event) => {
          const { container } = this.player.elements; // Retrieve the ad from the event. Some events (e.g. ALL_ADS_COMPLETED)
      ...
      constructor(player) {
        ...
        _defineProperty$1(this, "startMove", (event) => {
          ...
          if (
            ...
            return; // Wait until media has a duration
                  ...
                  if (newThumbFilename !== oldThumbFilename) {
                    ...
                    if (!this.loadedImages.includes(newThumbFilename)) {
                      ...
                      previewImage.onload = () => {
                        ...
                        if (!this.loadedImages.includes(newThumbFilename))
                          this.loadedImages.push(newThumbFilename); // We don't resolve until the thumb is loaded
        ...
        _defineProperty$1(this, "setThumbContainerPos", () => {
          ...
          const { container } = this.elements.thumb; // Find the lowest and highest desired left-position, so we don't slide out the side of the video container
          ...
          const max =
            ...
            10; // Set preview container position to: mousepos, minus seekbar.left, minus half of previewContainer.clientWidth
        ...
        _defineProperty$1(
          ...
          (previewImage, frame) => {
            ...
            }px`; // eslint-disable-next-line no-param-reassign
    ...
    class Plyr {
      ...
      get buffered() {
        const { buffered } = this.media; // YouTube / Vimeo return a float between 0-1
