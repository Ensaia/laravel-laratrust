...
const hijriDate = {
    ...
    currentMonthName: new Date().toLocaleString('en-US', {
        month: 'long'
    }),
    //Main Methods
    currentLanguage: 'en',
    ...
    hijriToString: function hijriToString(year, month, day) {
        hijriDate.outputHijriDate = `${this.addLeadingZero(day)} ${this.hijriMonths[(month)]} ${year} هجري`
    },
    outputGregorianDate: '',
    gregorianToString: function gregorianToString(year, month, day) {
        hijriDate.outputGregorianDate = `${this.weekDays[this.currentDayName]} ${year}/${this.addLeadingZero(month)}/${this.addLeadingZero(day)}`
        // hijriDate.outputGregorianDate = `${this.weekDays[this.currentDayName]} ${String(new date().getDate()).padStart(2, '0')}/${String(new date().getMonth() + 1).padStart(2, '0')}/${new date().getFullYear()}`
    },
    today: function today() {
        var today = new Date();
        this.gregorianToString(today.getFullYear(), (today.getMonth() + 1), today.getDate());
    ...
    toGregorian: function toGregorian(dateString, splitter) {
        ...
        if (arrDate.length >= 3)
            return hijriDate.hijriToGregorian(arrDate[2], arrDate[1], arrDate[0]);
    },
    toHijri: function toHijri(dateString, splitter) {
        ...
        var arrDate = dateString.split(splitter);
        if (arrDate.length >= 3)
            return hijriDate.gregorianToHijri(arrDate[2], arrDate[1], arrDate[0]);
    },
    hijriToGregorian: function hijriToGregorian(year, month, day) {
        ...
        var i = iln - 16260;
        var mcjdn = id + hijriDate.ummalqura_dat[i - 1] - 1;
    ...
    gregorianToHijri: function gregorianToHijri(pYear, pMonth, pDay) {
        ...
        for (var i = 0; i < hijriDate.ummalqura_dat.length; i++) {
            if (hijriDate.ummalqura_dat[i] > mcjdn)
                break;
        }
        ...
        var iy = ii + 1;
        var im = iln - 12 * ii;
        var id = mcjdn - hijriDate.ummalqura_dat[i - 1] + 1;
        var ml = hijriDate.ummalqura_dat[i] - hijriDate.ummalqura_dat[i - 1];
    ...
    HDate: function HDate(year, month, day) {
        ...
        this.month = month;
        this.day = day;
        hijriDate.hijriToString(this.year, this.month, this.day)
        ...
        this.format = function useFormat(year, month, day, format) {
            if (hijriDate.vlidateHijri(year, month, day)) {
                ...
                newFormat = newFormat.replace("N", hijriDate.lang[hijriDate.currentLanguage].notation);
                return hijriDate.lang[hijriDate.currentLanguage].formatLocale(newFormat);
            }
        }
    },
    ...
    lang: {
        'en': {
            ...
            formatLocale: function(hDate) { // For English it will convert from currentLanguages numbers to Arabic numbers.
                /*
                 if (hijriDate.currentLanguage != 'en') {
                 for ( i = 0; i < hDate.length; i++) {
                 switch(hDate[i]) {
                 case Hijrijs.lang[hijriDate.currentLanguage].zero:
                 hDate[i] = Hijrijs.lang['en'].zero;
                 break;
                 case Hijrijs.lang[hijriDate.currentLanguage].one:
                 hDate[i] = Hijrijs.lang['en'].one;
                 break;
                 case Hijrijs.lang[hijriDate.currentLanguage].two:
                 hDate[i] = Hijrijs.lang['en'].two;
                 break;
                 case Hijrijs.lang[hijriDate.currentLanguage].three:
                 hDate[i] = Hijrijs.lang['en'].three;
                 break;
                 case Hijrijs.lang[hijriDate.currentLanguage].four:
                 hDate[i] = Hijrijs.lang['en'].four;
                 break;
                 case Hijrijs.lang[hijriDate.currentLanguage].five:
                 hDate[i] = Hijrijs.lang['en'].five;
                 break;
                 case Hijrijs.lang[hijriDate.currentLanguage].six:
                 hDate[i] = Hijrijs.lang['en'].six;
                 break;
                 case Hijrijs.lang[hijriDate.currentLanguage].seven:
                 hDate[i] = Hijrijs.lang['en'].seven;
                 break;
                 case Hijrijs.lang[hijriDate.currentLanguage].ehgit:
                 hDate[i] = Hijrijs.lang['en'].ehgit;
                 break;
                 case Hijrijs.lang[hijriDate.currentLanguage].nine:
...
const printedDate = `<span class="week-day">${hijriDate.outputGregorianDate}</span> - <span class="hijri-date">${hijriDate.outputHijriDate}</span>`;
